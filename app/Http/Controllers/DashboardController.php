...
<span style="font-weight: lighter"> <b class="bold-size" style="font-weight: bold !important; font-size: 16px; text-transform: uppercase;"> TOTAL COBRANZA - ' . Carbon::parse($date_pagos)->monthName . ' :  ' . $object_totales['progress_pagos'] . '%</b> - ' . $object_totales['total_pagado'] . '/' . $object_totales['total_pedido_mespasado'] . '</span></div>';
    ...
    <div class="position-absolute w-100 text-center rounded h-40 h-60-res height-bar-progress top-progress-bar-total" style="top: 3px !important;height: 30px !important;font-size: 12px;">
        ...
        /*LUISSSSSSSSSSSSSSSSSSSSSSSSSSSSS ----- 46   */
            ...
            if ($count_asesor[46]['total_pedido_mespasado'] == 0) {
                ...
                if (($count_asesor[46]['total_pedido_mespasado']) >0)
                {
                    $round=round( ( ($count_asesor[46]['total_pagado'])/$count_asesor[46]['total_pedido_mespasado'] )*100 ,2);
                }else{
                    $round=0.00;
                }
            ...
            if ($count_asesor[46]['meta'] == 0) {
                ...
                if($count_asesor[46]['total_pedido']>0)
                {
                    $round=round( ( ($count_asesor[46]['total_pedido'])/$count_asesor[46]['meta'] )*100 ,2);
                }else{
                    $round=0.00;
                    //cuando pedidos es 0
                }
    ...
    <div class="position-absolute w-100 text-center rounded h-40 h-60-res height-bar-progress top-progress-bar-total" style="top: 3px !important;height: 30px !important;font-size: 12px;">
            ...
            if ($count_asesor[24]['total_pedido_mespasado'] == 0) {
                ...
                if (($count_asesor[24]['total_pedido_mespasado']) >0)
                {
                    $round=round( ( ($count_asesor[24]['total_pagado'])/$count_asesor[24]['total_pedido_mespasado'] )*100 ,2);
                }else{
                    $round=0.00;
                }
            ...
            if ($count_asesor[24]['meta'] == 0) {
                ...
                if($count_asesor[24]['total_pedido']>0)
                {
                    $round=round( ( ($count_asesor[24]['total_pedido'])/$count_asesor[24]['meta'] )*100 ,2);
                }else{
                    $round=0.00;
                    //cuando pedidos es 0
                }
    ...
    <div class="position-absolute w-100 text-center rounded h-40 h-60-res height-bar-progress top-progress-bar-total" style="top: 3px !important;font-size: 12px; height: 30px !important;">
            ...
            $html .= '<thead>
                <tr>
                    ...
                    <th width="33%">Dejaro de pedir  ' . Carbon::parse($date_pagos)->monthName . ' </th>
                </tr>
                </thead>
                <tbody>';
            foreach ($progressData as $data) {
            ...
            if($data["all_situacion_recurrente"]==0)
            {
                ...
                $porcentaje=($data["all_situacion_activo"] / $data["all_situacion_recurrente"])*100;
            }
                    ...
                    $html .= '<div class="w-100 bg-white rounded">
                              <div class="position-relative rounded">
                                  ...
                                  <div class="position-absolute rounded w-100 text-center" style="top: 5px;font-size: 16px;">
                                      <span style="font-weight: lighter"> <b class="bold-size" style="color:#001253;font-size:16px;">   ' . $porcentaje . '% - <span style="font-size:11px;color:grey;">'.$data["all_situacion_activo"].'/'.$data["all_situacion_recurrente"].'</span> </b>  <p class="text-red d-inline format-size" style="color: #d9686!important"> </p></span>
                                  </div>
                              </div>
                              <sub class="d-none">% -  Pagados/ Asignados</sub>
                            </div>';
