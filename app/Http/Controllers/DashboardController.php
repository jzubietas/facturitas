    ...
    public function viewMetaTable(Request $request)
    {
        ...
        foreach ($asesores as $asesor) {
            ...
            if ($asesor->excluir_meta) {
                ...
                else if ($total_pedido>=$metatotal_intermedia && $total_pedido < $metatotal_1) {
                    if ($metatotal_1 > 0) {
                        $p_pedidos = round(($total_pedido / $metatotal_1) * 100, 2);
                        $item['meta_new'] = 1;
...
<span style="font-weight: lighter"> <b class="bold-size" style="font-weight: bold !important; font-size: 16px; text-transform: uppercase;"> TOTAL COBRANZA - ' . Carbon::parse($date_pagos)->monthName . ' :  ' . $object_totales['progress_pagos'] . '%</b> - ' . $object_totales['total_pagado'] . '/' . $object_totales['total_pedido_mespasado'] . '</span></div>';
            ...
            //40 50 70 80 100 <
            if($object_totales['progress_pedidos']<=40)
            {
            ...
            foreach ($progressData as $data) {
             ...
             <td>';
                ...
                $sub_html='<sub class="top-visible" style="display: block !important;">
                                      <span class="badge bg-info font-'.$font_size_sub.'">M.Qui. '.$data["meta_quincena"].'</span>
                                      <span class="badge bg-gradient-warning font-'.$font_size_sub.'">M.Int. '.$data["meta_intermedia"].'</span>
                                      <span class="badge bg-gradient-purple font-'.$font_size_sub.'"">M.Pri. '.$data["meta"].'</span>
                                      <span class="badge bg-gradient-success font-'.$font_size_sub.'"">M.Seg. '.$data["meta_2"].'</span>
                                  </sub>';
                if($data["meta_new"]=='0')
                {
                    //quincena
                    //$html .=' el progreso de pedidos quicena '.$data["progress_pedidos"];
                    ...
                    if (  0<=$data["progress_pedidos"] && $data["progress_pedidos"] < 34)
                    {
                        $html .= '<div class="w-100 bg-white rounded">
                              <div class="position-relative rounded">
                                  ...
                                  <div class="position-absolute rounded w-100 text-center" style="top: 5px;font-size: 12px;">
                                      <span style="font-weight: lighter"> <b class="bold-size">  ' . $data["progress_pedidos"] . '% </b> - ' . $data["total_pedido"] . ' /' . $data["meta"] . '  <p class="text-red d-inline format-size" style="font-size: 18px; color: #d9686!important"> ' . ((($data["meta"] - $data["total_pedido"]) > 0) ? ($data["meta"] - $data["total_pedido"]) : ($data["meta_intermedia"] - $data["total_pedido"])) . '</p></span>
                                  </div>
                              </div>
                            </div>
                            '.$sub_html;
                        ...
                        $html .= '<div class="w-100 bg-white rounded">
                                    <div class="position-relative rounded">
                                      <div class="progress bg-white rounded height-bar-progress" style="height: 30px !important">
                                        ...
                                        <div class="position-absolute rounded w-100 text-center" style="top: 5px;font-size: 12px;">
                                            <span style="font-weight: lighter"> <b class="bold-size">  ' . $data["progress_pedidos"] . '% </b> - ' . $data["total_pedido"] . ' / ' . $data["meta_2"] . '<p class="text-red d-inline format-size" style="color: #d9686!important"> ' . ((($data["meta_2"] - $data["total_pedido"]) > 0) ? ($data["meta"] - $data["total_pedido"]) : ($data["meta"] - $data["total_pedido"])) . '</p></span>
                                        </div>
                                    </div>
                                  </div>
                                  '.$sub_html;
                        ...
                        $html .= '<div class="w-100 bg-white rounded">
                                    <div class="position-relative rounded">
                                      <div class="progress bg-white rounded height-bar-progress" style="height: 30px !important">
                                        ...
                                        <div class="position-absolute rounded w-100 text-center" style="top: 5px;font-size: 12px;">
                                            <span style="font-weight: lighter"> <b class="bold-size">  ' . $data["progress_pedidos"] . '% </b> - ' . $data["total_pedido"] . ' / ' . $data["meta_2"] . '<p class="text-red d-inline format-size" style="font-size: 18px; color: #d9686!important"> ' . ((($data["meta_2"] - $data["total_pedido"]) > 0) ? ($data["meta"] - $data["total_pedido"]) : ($data["meta"] - $data["total_pedido"]) ) . '</p></span>
                                        </div>
                                    </div>
                                  </div>
                                  '.$sub_html;
