...
class GraficoMetasDelMes extends Widgets
{
    ...
    public function generarDataNoviembre($date)
    {
        ...
        foreach ($asesores as $asesor) {
            ...
            if ($asesor->excluir_meta) {
                if ($metatotal > 0) {
                    $p_pedidos = round(($asignados / $metatotal) * 100, 2);
                } else {
                    $p_pedidos = 0;
                }
                if ($metatotal_2 > 0) {
                    $p_pedidos_2 = round(($asignados / $metatotal_2) * 100, 2);
                } else {
                    $p_pedidos_2 = 0;
                }
                ...
                $item['progress_pedidos_2'] = $p_pedidos_2;
                $item['progress'] = $p;
                $item['progress_2'] = $p_2;
                $item['progreso']= ($p_pedidos);
    ...
    /*public function generarDataDiciembre()
    {
        ...
        $dicResult = collect($newData)->values()->map(function ($item) {
            ...
            $item['progreso']= $p;
            return $item;
        })->sortBy('progreso',SORT_NUMERIC,SORT_DESC)->all();
