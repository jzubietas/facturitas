...
@section('title', 'Lista de Clientes en situacion PRETENDIDO')
...
@section('content_header')
    <h1>Lista de clientes en situacion PRETENDIDO
        @can('clientes.create')
        ...
        @can('clientes.exportar')
            ...
            @include('clientes.modal.exportar_unico', ['title' => 'Exportar Lista de clientes PRETENDIDO', 'key' => '5'])
        @endcan
    </h1>
    @if($superasesor > 0)
        <br>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#tablaPrincipal').DataTable({
                ...
                autowidth: true,
                serverSide: true,
                ajax: "{{ route('clientespretendidotabla') }}",
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        sWidth: '20%',
                        render: function (data, type, row, meta) {
                            var urledit = '{{ route("clientes.edit.pretendido", ":id") }}';
                            urledit = urledit.replace(':id', row.id);

                            var urlshow = '{{ route("clientes.show", ":id") }}';
                            urlshow = urlshow.replace(':id', row.id);

                            @can('clientes.edit.pretendido')
