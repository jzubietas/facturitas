...
@section('content_header')
    <h1>DETALLE DEL <b>PAGO</b>:PAG{{ $pago->users }}-{{$pago->cantidad_voucher}}{{$pago->cantidad_pedido}}
        -{{ $pago->id }}</h1>
    ...
    <div class="card">
                ...
                <div class="form-row">
                    <table class="table table-active">
                        ...
                        <tr>
                            ...
                            <th scope="col">{{ $pago->users }}</th>
                            </td>
                            <td>
                            <th scope="col" class="col-lg-2" style="text-align: right;">CLIENTE:</th>
                            <th scope="col">{{ $pago->celular }} - {{ $pago->nombre }}</th>
                            </td>
                            {{-- <td>
                              ...
                              <th scope="col">
                                <select name="condicion" class="border form-control selectpicker border-secondary" id="condicion" data-live-search="true">
                                  ...
                                  <option value="{{ $condicion }}" {{ ($condicion == $pago->condicion ? "selected" : "") }}>{{$condicion}}</option>
                                    @endforeach
                                </select>
                              </th>
                            </td> --}}
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped">
                                ...
                                @foreach ($pagoPedidos as $pagoPedido)
                                    <tr>
                                        <td>{{ $contPe + 1 }}</td>
                                        <td>PED000{{ $pagoPedido->pedidos }}<input type="hidden" name="pedido_id[]"
                                                                                   id="pedido_id"
                                                                                   value="{{ $pagoPedido->pedidos }}">
                                        </td>
                                        ...
                                        @if($pago->isAbonado())
                                            ...
                                            @endif
                                        @elseif($pago->isObservado())
                                            @if($pagoPedido->pagado == 1)
                                                <td>ADELANTO OBSERVADO</td>
                                            @else
                                                <td>PAGADO OBSERVADO</td>
                                            @endif
                                        @elseif($pago->isPendiente())
                                        ...
                                        <td><input type="hidden" name="pedido_id_abono[]" id="pedido_id_abono"
                                                   value="{{ $pagoPedido->abono }}">{{ $pagoPedido->abono }}</td>
                                        ...
                                        @endif
                                        <td>
                                            <a href="" data-target="#modal-historial-pagos-pedido" data-toggle="modal"
        ...
        <div class="card-body">
            <div class="border rounded card-body border-secondary">
                <div class="form-row">
                    <div class="form-group col-lg-12">
                        <h3 style="text-align: center"><strong>PAGOS REALIZADOS POR EL
                                CLIENTE</strong> @if($pago->saldo>0)
                                SALDO A FAVOR DEL CLIENTE: {{ $pago->saldo }}
                            @endif</h3>
                       ...
                       <span><?php
                           ...
                           <?php } ?>  </span>
                                            ...
                                            <input type="hidden" name="detalle_id[]" value="{{ $detallePago->id }}"
                                        ...
                                        <td>
                                            ...
                                            <p>
                                                <br><a href="" data-target="#modal-imagen-{{ $detallePago->id }}"
                                                       data-toggle="modal">
                                                    <img src="{{ asset('storage/pagos/' . $detallePago->imagen) }}"
                                                         alt="{{ $detallePago->imagen }}" height="200px" width="200px"
                                                         class="img-thumbnail" id="imagen_{{ $contPa + 1 }}">
                                                </a>

                                            </p>
                                            <a href="" data-target="#modal-cambiar-imagen" data-toggle="modal"
                                               data-imagen="{{ $detallePago->imagen }}"
                                               data-conciliar="{{ $detallePago->id }}" data-item="{{ $contPa + 1 }}">
                                                <button class="btn btn-danger btn-md">Cambiar</button>
                                            </a>
                                            <input type="hidden" value="" name="conciliar[]" class="conciliar_count"
                                                   id="conciliar_{{ $contPa + 1 }}">
                                        </td>
                                        <td>

                                            <p>
                                                <br>
                                                <a href="{{ route('pagos.descargarimagen', $detallePago->imagen) }}"
                                                   class="text-center">
                                                    <button type="button" class="btn btn-secondary btn-md"> Descargar
                                                    </button>
                                                </a>

                                                <a href="" data-target="#modal-conciliar-get"
                                                   class="modal-conciliar-get"
                                                   data-fechadeposito="{{ $detallePago->fecha_deposito }}"
                                                   data-toggle="modal" data-conciliar="{{ $detallePago->id }}"
                                                   data-item="{{ $contPa + 1 }}">
                                                    <button class="btn btn-danger btn-md">Conciliar</button>
                                                </a>
                                                <a href="" data-target="#modal-editar-get" data-toggle="modal"
                                                   data-pbanco="{{ $detallePago->banco }}"
                                                   data-titulares="{{ $detallePago->titular }}"
                                                   data-fecha="{{ $detallePago->fecha_deposito_change }}"
                                                   data-conciliar="{{ $detallePago->id }}"
                                                   data-item="{{ $contPa + 1 }}">
                                ...
                                <tr>
                                    ...
                                    <td colspan="3">
                                        {!! Form::textarea('observacion', $pago->observacion, ['class' => 'form-control', 'rows' => '7', 'placeholder' => 'Ingrese observaciones']) !!}
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <th>
                                        ...
                                        <select name="condicion"
                                                class="border form-control selectpicker border-secondary d-none"
                                                id="condicion" data-live-search="true">
                                            <option value="">---- SELECCIONE ----</option>
                                            @foreach($condiciones as $condicion)
                                                <option
                                                    value="{{ $condicion }}" {{ ($condicion == $pago->condicion ? "selected" : "") }}>{{$condicion}}</option>
                                            @endforeach
                                        </select>
        ...
        <div class="card-footer text-center" id="guardar">
            <div class="row">
                ...
                <div class="col-10">
                    <button type="button" id="aprobarrbtn" class="btn btn-success btn-lg"><i class="fas fa-save"></i>
                        APROBAR
                    </button>
                    <button type="button" id="observarbtn" class="btn btn-danger btn-lg"><i class="fas fa-save"></i>
                        OBSERVADO
                    </button>
                    <button type="button" id="pendientebtn" class="btn btn-warning btn-lg"><i class="fas fa-save"></i>
                        PENDIENTE
                    </button>
                </div>
            </div>
...
//#modal-imagen-4359 modalimagen_item
            ...
            $(document).on("submit", "#formulario", function (event) {
                ...
                if (campo_condicion == '{{\App\Models\Pago::ABONADO}}') {
                    ...
                    console.log("subcondicion:  {{$pago->subcondicion}}")

                    @if($pago->subcondicion!='COURIER PERDONADO')
